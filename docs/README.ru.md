# Документация Nrgy.js

## Назначение

Эта директория является основной продуктовой документацией Nrgy.js. Она
объясняет, как использовать библиотеку, как устроена её архитектурная модель и
как вносить изменения в репозиторий.

## Продуктовая документация

- [Введение](./introduction.ru.md)
- [Быстрый старт](./quick-start.ru.md)
- [Core](./core/README.ru.md)
- [Архитектура](./architecture/README.ru.md)
- [MVVM и Controllers](./mvvm/README.ru.md)
- [Интеграции](./integrations/README.ru.md)
- [Рецепты](./recipes/README.ru.md)
- [Миграция](./migration/README.ru.md)
- [FAQ](./faq/README.ru.md)
- [English version](./README.md)

## Рекомендуемый порядок чтения

1. Начать с [Введения](./introduction.ru.md).
2. Затем перейти к [Быстрому старту](./quick-start.ru.md).
3. После этого пройти [Core](./core/README.ru.md) и
   [Архитектуру](./architecture/README.ru.md).
4. Затем перейти к [MVVM и Controllers](./mvvm/README.ru.md).
5. Во время внедрения использовать [Интеграции](./integrations/README.ru.md),
   [Рецепты](./recipes/README.ru.md), [Миграцию](./migration/README.ru.md) и
   [FAQ](./faq/README.ru.md).

## Contributing

- [Onepager для агентов](../AGENTS.ru.md) (стартовая точка для AI-агентов)
- [Обзор для контрибьюторов](./contributing/README.ru.md)
- [Требования к документации](./contributing/docs_requirements.ru.md)
- [Стиль кодирования](./contributing/coding_style.ru.md)
- [Процесс разработки](./contributing/development_workflow.ru.md)
- [Релизный процесс](./contributing/release_workflow.ru.md)

## Документация пакетов

- [@nrgyjs/core](../packages/core/README.ru.md)
- [@nrgyjs/react](../packages/react/README.ru.md)
- [@nrgyjs/ditox](../packages/ditox/README.ru.md)
- [@nrgyjs/ditox-react](../packages/ditox-react/README.ru.md)
- [@nrgyjs/rxjs](../packages/rxjs/README.ru.md)
- [@nrgyjs/rx-effects](../packages/rx-effects/README.ru.md)

## Примечания

- Обзоры публичного API на уровне пакета живут в `README.md` каждого пакета.
- Документация модулей colocated с исходниками внутри `packages/*/src`.
- Держите английскую и русскую документацию синхронизированной.
