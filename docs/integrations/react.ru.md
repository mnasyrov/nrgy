...
Оба хука построены на `useSyncExternalStore()` и требуют React 18 или новее.
Обновления доставляются синхронно: обновление внутри обработчика события
рендерится без ожидания микрозадачи, а смена atom, переданного в хук, видна в
том же коммите.
...
`useAtoms()` нужен, когда компонент читает несколько atoms как один объект.
Объект atoms можно создавать прямо в рендере: хук сохраняет результат
стабильным, пока atoms и их значения не меняются.
