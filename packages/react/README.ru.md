...
Пакет требует React 18 или новее.

## Концептуальная архитектура

Пакет строится вокруг деклараций из `@nrgyjs/core` и жизненного цикла React.

1. `useAtom()` подписывает компонент на один `Atom<T>` через
   `useSyncExternalStore()` и доставляет обновления синхронно.
2. `useAtoms()` собирает объект атомов в один вычисляемый атом и сохраняет
   стабильность результата через структурное сравнение. Объект атомов можно
